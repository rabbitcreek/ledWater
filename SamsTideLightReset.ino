...
void tempTime(){
  if(WiFi.status()== WL_CONNECTED){
      String serverPath = "http://api.openweathermap.org/data/2.5/weather?lat=61.095630&lon=-149.786870&APPID=e28ba1db8ba3c57983a446d6afbcb55b";
      ...
      int sunUp = int(myObject["sys"]["sunrise"]);
      int sunDown = int(myObject["sys"]["sunset"]);
       hourLight = (sunDown - sunUp)/3600;
...
void sunLight(){
  for(int i = 0; i< 60; i++){
    int colorEye  =  map(i, 0,59, 1, 255);
    leds[i] = ColorFromPalette(RainbowColors_p, colorEye, 255, LINEARBLEND); 
  }
  delay (1000);
  Serial.print("hourLight");
  Serial.print(hourLight);
  int numberLeft = (float(hourLight) * 2.5)/2;
  Serial.print("numberLeft");
  Serial.println(numberLeft);
  for(int i = 0; i < 30 - numberLeft; i++){
    leds[i] = CRGB::DarkBlue;
    leds[60 - i] = CRGB::DarkBlue;
    FastLED.show();
    delay(100);
  }
  delay(1000);
