...
void fallTemp(){
  ...
  if (pos <=  pMax) {
    ...
    for(int j=0;j<5;j++){
    //strip.setPixelColor(pos + j,255,0,0);
    leds[pos + j] = CRGB::Red;
   ...
   //for(int j = 0 ;j < pMax;j++)strip.setPixelColor(j,250,0,0);
   for(int j = 0 ;j < pMax;j++)leds[j] = CRGB::Red;
   //for(int j = 0 ; j <= pMax; j = j +5)strip.setPixelColor(j, 0,100,0);
   for(int j = 0 ; j <= pMax; j = j +5)leds[j] = CRGB::Green;
    FastLED.show();
   
   //for(int j = pMax ;j < 60;j++)strip.setPixelColor(j,0,0,0);
 ...
 delay(5000);
  ...
  if (pos <=  pMax) {
    ...
    for(int j=0;j<5;j++){
    //strip.setPixelColor(pos + j,0,0,255);
   ...
   //for(int j = 0 ;j < pMax;j++)strip.setPixelColor(j,0,0,250);
   for(int j = 0 ;j < pMax;j++)leds[j] = CRGB::Blue;
   //for(int j = 0 ; j <= pMax; j = j +5)strip.setPixelColor(j, 0,255,0);
   for(int j = 0 ; j <= pMax; j = j +5)leds[j] = CRGB::Green;
